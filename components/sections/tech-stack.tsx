...
export function TechStack() {
                    ...
                    {technologies.map((tech, index) => {
                      ...
                      return (
                        <div
                          ...
                          className={`relative flex flex-col items-center justify-center gap-1 sm:gap-2 p-2 sm:p-3 rounded-xl backdrop-blur-md border transition-all duration-300 cursor-pointer ${isHovered
                    ...
                    {technologies.map((tech, index) => {
                      ...
                      return (
                        <div
                          ...
                          className={`relative flex flex-col items-center justify-center gap-2 p-4 rounded-2xl backdrop-blur-md border transition-all duration-300 cursor-pointer ${isHovered
              ...
              return (
                <div key={index} className="relative">
                  <div
                    ...
                    onMouseLeave={() => !isTouchDevice && setHoveredTech(null)}
                    onClick={() => isTouchDevice && setHoveredTech(hoveredTech === tech.name ? null : tech.name)}
                    className={`relative w-[280px] sm:w-[500px] h-16 sm:h-20 backdrop-blur-xl border-2 rounded-xl flex items-center gap-3 sm:gap-6 px-3 sm:px-6 transition-all duration-300 cursor-pointer ${isHovered
                  ...
                  >
                    <div className={`w-8 h-8 sm:w-12 sm:h-12 rounded-full border-2 flex items-center justify-center ${isHovered ? "border-green-400" : "border-green-400/30"
                      }`}>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex gap-1.5 mt-1.5">
                        {[...Array(5)].map((_, i) => (
                          <div
                            key={i}
                            className={`w-2 h-2 rounded-full transition-all ${isHovered ? "bg-green-400 animate-pulse" : "bg-green-500/30"
                              }`}
                    ...
                    <div className="hidden sm:flex flex-col items-end gap-1">
                      <div className={`text-[10px] font-mono px-2 py-1 rounded border ${isHovered
                  ...
                  return (
                    <div key={index}>
                      <div
                        onMouseEnter={() => setHoveredTech(tech.name)}
                        onMouseLeave={() => setHoveredTech(null)}
                        className={`relative h-[${sectionHeight}px] flex items-center justify-center gap-4 cursor-pointer transition-all duration-300 ${isHovered ? "bg-purple-500/25" : "bg-transparent hover:bg-purple-500/10"
                          }`}
                    ...
                    return (
                      <div key={index}>
                        <div
                          onMouseEnter={() => setHoveredTech(tech.name)}
                          onMouseLeave={() => setHoveredTech(null)}
                          className={`absolute flex flex-col items-center justify-center gap-2 p-4 rounded-xl backdrop-blur-xl border-2 transition-all duration-300 cursor-pointer ${isHovered
                  ...
                  {technologies.map((tech, index) => {
                    ...
                    return (
                      <div
                        ...
                        onMouseLeave={() => !isTouchDevice && setHoveredTech(null)}
                        onClick={() => isTouchDevice && setHoveredTech(hoveredTech === tech.name ? null : tech.name)}
                        className={`relative flex flex-col items-center justify-center gap-2 sm:gap-3 p-3 sm:p-5 rounded-2xl backdrop-blur-xl border-2 transition-all duration-300 cursor-pointer ${isHovered
  ...
  return (
    <section ref={sectionRef} className="relative min-h-screen py-20 bg-background overflow-hidden">
      ...
      >
        <motion.div
          className="absolute top-0 right-1/4 w-[600px] h-[600px] bg-primary/5 rounded-full blur-[120px]"
          animate={{
            scale: [1, 1.2, 1],
            opacity: [0.3, 0.5, 0.3]
          }}
        ...
        />
        <motion.div
          className="absolute bottom-0 left-1/4 w-[500px] h-[500px] bg-accent/5 rounded-full blur-[100px]"
          animate={{
            scale: [1, 1.3, 1],
            opacity: [0.3, 0.5, 0.3]
          }}
            ...
            {categories.map((category, idx) => {
              ...
              return (
                <motion.button
                  ...
                  className={`flex items-center gap-3 px-6 py-4 rounded-xl border-2 font-semibold transition-all duration-300 focus:outline-none cursor-pointer ${isSelected
                    ? `border-primary ${category.bgColor} text-white shadow-lg shadow-primary/30`
                    : "border-border/30 bg-card text-foreground hover:border-primary/50"
                    }`}
                  initial={{ opacity: 0, scale: 0.8, y: 20 }}
                  animate={isInView ? { opacity: 1, scale: isSelected ? 1.05 : 1, y: 0 } : {}}
                  transition={{ delay: 0.9 + idx * 0.05, duration: 0.4, ease: "backOut" }}
              ...
              {categories.map((category, idx) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`shrink-0 flex items-center gap-2 px-4 py-3 rounded-xl border-2 font-semibold transition-all duration-300 focus:outline-none whitespace-nowrap cursor-pointer ${isSelected
                      ? `border-primary ${category.bgColor} text-white shadow-lg shadow-primary/30`
                      : "border-border/30 bg-card text-foreground"
                      }`}
                    initial={{ opacity: 0, x: -20 }}
                    animate={isInView ? { opacity: 1, x: 0, scale: isSelected ? 1.05 : 1 } : {}}
