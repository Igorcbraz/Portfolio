...
import { useInView, fadeInUp, staggerContainer } from "@/hooks/use-animations"
...
export function CodeDashboard() {
  ...
  const { ref: cardsRef, isInView: cardsInView } = useInView({ threshold: 0.2, triggerOnce: true })
  ...
  return (
    <section className="relative min-h-screen py-20 bg-background">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div
          className="absolute right-0 top-1/4 w-[400px] h-[400px] bg-primary/5 rounded-full blur-[100px]"
          animate={{
            scale: [1, 1.2, 1],
            opacity: [0.3, 0.5, 0.3],
          }}
      ...
      <div ref={sectionRef} className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <motion.div
            className="bg-card border border-border/30 rounded-lg p-6 flex flex-col"
            whileHover={{
              scale: 1.02,
                  ...
                  >
                    ...
                    <div className="h-3 bg-muted/30 rounded-full overflow-hidden">
                      <motion.div
                        ...
                        initial={{ width: 0 }}
                        animate={statsInView ? { width: `${percentage}%` } : { width: 0 }}
                        transition={{
                          duration: 1,
                          delay: idx * 0.1 + 0.5,
                          ease: [0.22, 1, 0.36, 1]
          ...
          {reposLoading ? (
            ...
            currentRepos.map((repo, index) => (
              <motion.a
                ...
                className="group relative bg-card border border-border/30 rounded-lg p-6 overflow-hidden"
                variants={fadeInUp}
                whileHover={{
                  y: -12,
                  scale: 1.03,
                ...
                transition={{
                  delay: (index % reposPerPage) * 0.1,
                  duration: 0.5,
                  ease: [0.22, 1, 0.36, 1]
                }}
              >
                <motion.div
                  className="absolute inset-0 bg-linear-to-br from-primary/5 to-accent/5"
                  initial={{ opacity: 0 }}
                  whileHover={{ opacity: 1 }}
                  transition={{ duration: 0.3 }}
                />
                ...
                <div className="relative z-10 space-y-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <motion.div
                        ...
                        transition={{ delay: (index % reposPerPage) * 0.1 + 0.2 }}
                      >
                        <motion.h3
                        ...
                        {repo.stars > 10 && (
                          <motion.span
                            className="shrink-0 px-2 py-0.5 bg-yellow-500/10 text-yellow-500 text-[10px] rounded-full font-medium"
                            initial={{ scale: 0, opacity: 0 }}
                            animate={{ scale: 1, opacity: 1 }}
                            transition={{
                              delay: (index % reposPerPage) * 0.1 + 0.3,
                              type: "spring",
                              stiffness: 500,
                              damping: 20
                            }}
                      ...
                      <motion.p
                        ...
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        transition={{ delay: (index % reposPerPage) * 0.1 + 0.25 }}
                      >
                        {repo.description}
                      </motion.p>
                    </div>
                    <motion.div
                      className="shrink-0 w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-primary-foreground transition-all"
                  ...
                  {repo.topics.length > 0 && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: (index % reposPerPage) * 0.1 + 0.3 }}
                    >
                      {repo.topics.slice(0, 3).map((topic, i) => (
                        <motion.span
                          ...
                          animate={{ scale: 1, opacity: 1 }}
                          transition={{
                            delay: (index % reposPerPage) * 0.1 + 0.35 + (i * 0.05),
                            type: "spring",
                            stiffness: 400,
                            damping: 15
                          }}
                      ...
                      {repo.topics.length > 3 && (
                        <motion.span
                          className="px-2 py-0.5 bg-muted/50 text-muted-foreground text-[10px] rounded-md"
                          initial={{ scale: 0 }}
                          animate={{ scale: 1 }}
                          transition={{
                            delay: (index % reposPerPage) * 0.1 + 0.5,
                            type: "spring"
                          }}
                  ...
                  <motion.div
                    ...
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    transition={{ delay: (index % reposPerPage) * 0.1 + 0.4 }}
                  >
                    {[
                      {
                        icon: <svg className="w-4 h-4 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
                          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                        </svg>, value: repo.stars, label: dictionary.code.smallLabels.stars
                      },
                      {
                        icon: <svg className="w-4 h-4 text-blue-500" fill="currentColor" viewBox="0 0 16 16">
                          <path fillRule="evenodd" d="M5 3.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm0 2.122a2.25 2.25 0 10-1.5 0v.878A2.25 2.25 0 005.75 8.5h1.5v2.128a2.251 2.251 0 101.5 0V8.5h1.5a2.25 2.25 0 002.25-2.25v-.878a2.25 2.25 0 10-1.5 0v.878a.75.75 0 01-.75.75h-4.5A.75.75 0 015 6.25v-.878zm3.75 7.378a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm3-8.75a.75.75 0 100-1.5.75.75 0 000 1.5z" />
                        </svg>, value: repo.forks, label: dictionary.code.smallLabels.forks
                      },
                      {
                        icon: <svg className="w-4 h-4 text-green-500" fill="currentColor" viewBox="0 0 20 20">
                          <path d="M10 12a2 2 0 100-4 2 2 0 000 4z" />
                          <path fillRule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clipRule="evenodd" />
                        </svg>, value: repo.watchers, label: dictionary.code.smallLabels.watch
                      },
                      {
                        icon: <svg className="w-4 h-4 text-red-500" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                        </svg>, value: repo.openIssues, label: dictionary.code.smallLabels.issues
                      }
                    ].map((stat, statIdx) => (
                      <motion.div
                        key={statIdx}
                        ...
                        initial={{ opacity: 0, x: -10 }}
                        animate={{ opacity: 1, x: 0 }}
                        transition={{ delay: (index % reposPerPage) * 0.1 + 0.45 + (statIdx * 0.05) }}
                  ...
                  <motion.div
                    ...
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: (index % reposPerPage) * 0.1 + 0.6 }}
                  >
                    <div className="flex items-center gap-3">
                      <motion.span
                      ...
                      <motion.span
                        ...
                        whileHover={{ scale: 1.05, color: "rgba(var(--foreground-rgb), 0.8)" }}
                      >
